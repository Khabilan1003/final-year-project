...
def patentOfficeValidatePatent():
    ...
    serialized_abstract_embeddings = dict(current_document).get('abstract_embeddings')
    abstract_embeddings = pickle.loads(serialized_abstract_embeddings)

    # Normalizing the embed vector to unit vector
    abstract_embeddings_distance = np.sqrt((abstract_embeddings * abstract_embeddings).sum())
    abstract_embeddings = abstract_embeddings / abstract_embeddings_distance

    cursor = collection_patents.find({"title" : {"$not" : {"$eq" : title}} , "patent_status" : PatentStatus.APPROVED.value} , {"abstract_embeddings" : 1})
    documents = []

    for document in cursor:
        serialized_document_embeddings = dict(document).get("abstract_embeddings")
        document_embeddings = pickle.loads(serialized_document_embeddings)
        document_embeddings_distance = np.sqrt((document_embeddings * document_embeddings).sum())
        document_embeddings = document_embeddings / document_embeddings_distance

        probability = document_embeddings.dot(abstract_embeddings)
